#include "TrafficFlowReordering.h"

Routing::Algorithms::TrafficFlowReordering::TrafficFlowReordering(
        const shared_ptr<Routing::Data::GraphMemory> &routingGraph, const float beta, const float gamma)
        : ReorderingBase(routingGraph) {

}

vector<long long int>
Routing::Algorithms::TrafficFlowReordering::Reorder(const vector<Routing::Algorithms::ReorderInput> &reorderInput,
                                                    std::vector<std::pair<long long,
                                                            std::vector<long long>>> &lastRoute,
                                                    const long long int currentTmw, const long long int tmwSize) {
    ...
    for (size_t i = 0; i < reorderInput.size(); ++i) {
        ...
        for (const auto &s : reorderInput[i].segments) {
            ...
            for (const auto &t : s.second) {
                ...
                /**
                 * Overlap with last route returned over time
                 */
                if (currentRouteTmwIdx < lastRoute.size()) {
                    // Iterate over segments in last route for current tmw
                    // If current segment is found, increment overlap
                    long lastAnyOverlap = anyOverlapCount;
                    for (const auto &lastSeg : lastRoute[currentRouteTmwIdx].second) {
                        // Try to find the segment in last route in current tmw
                        if (lastSeg == s.first.edgeId) {
                            tmwOverlapCount++;
                            anyOverlapCount++;
                            //overlapCount[lastRoute[currentRouteTmwIdx].first] = overlap;
                            break;
                        }

                        // Try any other segments
                        // If current segment was not found in current tmw
                        if (anyOverlapCount == lastAnyOverlap) {
                            for (const auto &id : edgeIds) {
                                if (id == s.first.edgeId) {
                                    anyOverlapCount++;
                                    break;
                                }
                            }
        ...
        float routeOverlap = static_cast<float>(anyOverlapCount) / reorderInput[i].segments.size();
